...
from SGMGU.views.utiles import paginar, crear_lista_pages
...
@login_required
@permission_required(['administrador', 'dpts', 'especialista', 'organismo', 'mes', 'juridico', 'interrupto'])
...
@login_required
@permission_required(['administrador', 'dpts', 'especialista', 'organismo', 'mes', 'juridico'])
...
def gestion_usuarios(request):
    ...
    if q:
        usuarios = usuarios.filter(
            ...
            Q(perfil_usuario__categoria__nombre__icontains=q) |
            Q(perfil_usuario__organismo__nombre__icontains=q) |
            Q(perfil_usuario__organismo__siglas__icontains=q) |
            Q(perfil_usuario__provincia__nombre__icontains=q) |
            Q(perfil_usuario__municipio__nombre__icontains=q) |
            Q(perfil_usuario__telefono__icontains=q)
        )

    usuarios = paginar(request, usuarios)
    paginas = crear_lista_pages(usuarios)

    context = {'usuarios': usuarios, 'paginas': paginas}
