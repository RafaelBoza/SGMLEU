...
def tecnicos_medios_pendientes_por_no_existir_oferta_cierre_mes(request):
    ...
    response[
        'Content-Disposition'] = "attachment; filename=Tecnicos_medios_pendientes_por_no_existir_oferta._(%s).xlsx" % (
        anno)
    ...
    tecnicos_medio = TMedioOCalificadoEOficio.objects.filter(fuente_procedencia_id=6, causa_no_ubicado_id=1,
                                                            activo=True)
    ...
    for indice_provincia, provincia in enumerate(provincias):
        ...
        for indice_causa, especialidad in enumerate(especialidades):
            ...
            worksheet_data.write(indice_provincia + 1, indice_causa + 1,
                                 tecnicos_medio.filter(carrera_id=especialidad.id, municipio_solicita_empleo__provincia=provincia).count(),
                                 formato2)
