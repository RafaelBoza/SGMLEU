...
def total_personas_no_ubicadas_provincias(request):
    ...
    for provincia in provincias:
        ...
        total = (
                    licenciados_sma_controlados + egresados_ep_controlados + sancionados_controlados + desvinculados_controlados +
        ...
        licenciados_sma_controlados = licenciados_sma.filter(municipio_residencia__provincia=provincia,
                                                             sexo='F').count()
        egresados_ep_controlados = egresados_ep.filter(municipio_solicita_empleo__provincia=provincia, sexo='F').count()
        sancionados_controlados = sancionados.filter(municipio_solicita_empleo__provincia=provincia, sexo='F').count()
        ...
        tecnicos_medio_controlados = tecnicos_medio.filter(municipio_solicita_empleo__provincia=provincia,
                                                           edad__lte=35).count()
        obreros_calificados_controlados = obreros_calificados.filter(
            municipio_solicita_empleo__provincia=provincia, edad__lte=35).count()
        escuelas_oficio_controlados = escuelas_oficio.filter(municipio_solicita_empleo__provincia=provincia,
                                                             edad__lte=35).count()
        egresados_escuelas_especiales_controlados = egresados_escuelas_especiales.filter(
            municipio_solicita_empleo__provincia=provincia, edad__lte=35).count()
        ...
        hombres_riesgo_pnr_controlados = hombres_riesgo_pnr.filter(
            municipio_solicita_empleo__provincia=provincia, edad__lte=35).count()
        proxenetas_controlados = proxenetas.filter(municipio_solicita_empleo__provincia=provincia, edad__lte=35).count()
    ...
    worksheet_data.write_column(1, 7, cero, formato2)  # NO UBICADOS: LICENCIADOS DEL SMA
