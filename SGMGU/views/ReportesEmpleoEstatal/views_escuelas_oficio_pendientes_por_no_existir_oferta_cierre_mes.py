...
def escuelas_oficio_pendientes_por_no_existir_oferta_cierre_mes(request):
    ...
    response[
        'Content-Disposition'] = "attachment; filename=Escuelas_oficio_pendientes_por_no_existir_oferta._(%s).xlsx" % (
        anno)
    ...
    for persona in escuelas_oficio:
        if persona.carrera is None:
            pass
        elif not persona.carrera.id in ids_especialidades:
    ...
    for indice_provincia, provincia in enumerate(provincias):
        ...
        for indice_causa, especialidad in enumerate(especialidades):
            ...
            worksheet_data.write(indice_provincia + 1, indice_causa + 1,
                                 escuelas_oficio.filter(carrera_id=especialidad.id, municipio_solicita_empleo__provincia=provincia).count(),
                                 formato2)
