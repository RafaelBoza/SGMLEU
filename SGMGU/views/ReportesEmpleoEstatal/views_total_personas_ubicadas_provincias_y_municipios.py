...
def total_personas_ubicadas_provincias_y_municipios(request):
    ...
    egresados_ep = EgresadosEstablecimientosPenitenciarios.objects.filter(fuente_procedencia_id=2,
                                                                          activo=True). \
        values("municipio_residencia__provincia", "municipio_residencia", "sexo", "edad")

    sancionados = EgresadosEstablecimientosPenitenciarios.objects.filter(fuente_procedencia_id=3,
                                                                         activo=True). \
        values("municipio_solicita_empleo__provincia", "municipio_solicita_empleo", "sexo", "edad", "ubicado")
    ...
    if categoria_usuario == 'administrador' or categoria_usuario == 'dpt_ee':
        ...
        for provincia in provincias:
            ...
            egresados_ep_provincia = egresados_ep.filter(municipio_solicita_empleo__provincia=provincia, ubicado=True).count()
            sancionados_provincia = sancionados.filter(municipio_solicita_empleo__provincia=provincia, ubicado=True).count()
            desvinculados_provincia = desvinculados.filter(municipio_solicita_empleo__provincia=provincia,ubicado=True).count()
            tecnicos_medio_provincia = tecnicos_medio.filter(municipio_solicita_empleo__provincia=provincia,ubicado=True).count()
            obreros_calificados_provincia = obreros_calificados.filter(municipio_solicita_empleo__provincia=provincia,ubicado=True).count()
            escuelas_oficio_provincia = escuelas_oficio.filter(municipio_solicita_empleo__provincia=provincia,ubicado=True).count()
            egresados_escuelas_especiales_provincia = egresados_escuelas_especiales.filter(municipio_solicita_empleo__provincia=provincia, ubicado=True).count()
            egresados_escuelas_conducta_provincia = egresados_escuelas_conducta.filter(municipio_solicita_empleo__provincia=provincia, ubicado=True).count()
            egresados_efi_provincia = egresados_efi.filter(municipio_solicita_empleo__provincia=provincia,ubicado=True).count()
            menores_incapacitados_provincia = menores_incapacitados.filter(municipio_solicita_empleo__provincia=provincia,ubicado=True).count()
            menores_desvinculados_provincia = menores_desvinculados.filter(municipio_solicita_empleo__provincia=provincia,ubicado=True).count()
            menores_dictamen_provincia = menores_dictamen.filter(municipio_solicita_empleo__provincia=provincia,ubicado=True).count()
            discapacitados_provincia = discapacitados.filter(municipio_solicita_empleo__provincia=provincia,ubicado=True).count()
            mujeres_riesgo_pnr_provincia = mujeres_riesgo_pnr.filter(municipio_solicita_empleo__provincia=provincia,ubicado=True).count()
            hombres_riesgo_pnr_provincia = hombres_riesgo_pnr.filter(municipio_solicita_empleo__provincia=provincia,ubicado=True).count()
