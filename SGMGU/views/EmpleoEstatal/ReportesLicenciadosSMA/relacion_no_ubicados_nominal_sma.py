...
def relacion_no_ubicados_nominal_sma(request):
    ...
    if categoria_usuario == 'dmt':
        response[
            'Content-Disposition'] = "attachment; filename=Relacion_nominal_no_incorporados_sma(%s)_(%s).xlsx" % (municipio_usuario,
                                                                                                anno_actual)
        ...
        response['Content-Disposition'] = "attachment; filename=Relacion_nominal_no_incorporados_sma(%s)_(%s).xlsx" % (provincia_usuario,
                                                                                                     anno_actual)
        ...
        response['Content-Disposition'] = "attachment; filename=Relacion_nominal_no_incorporados_sma(%s).xlsx" % anno_actual

        LICENCIADOS_SMA = LicenciadosSMA.objects.filter(fecha_registro__year=anno_actual, incorporado_id=2).values("municipio_residencia__provincia__nombre", "nombre_apellidos", "ci", "municipio_residencia__nombre", "mes_entrevista", "causa_no_ubicado__causa") | \
                          LicenciadosSMA.objects.filter(activo=True, incorporado_id=2).values("municipio_residencia__provincia__nombre", "nombre_apellidos", "ci", "municipio_residencia__nombre", "mes_entrevista", "causa_no_ubicado__causa")
