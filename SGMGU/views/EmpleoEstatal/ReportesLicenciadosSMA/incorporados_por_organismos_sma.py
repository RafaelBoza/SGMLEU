...
def incorporados_por_organismos_sma(request):
    ...
    if categoria_usuario == 'dmt':
        response[
            'Content-Disposition'] = "attachment; filename=Incorporados_por_organismos_sma_(%s)_(%s).xlsx" % (municipio_usuario,
                                                                                                anno_actual)
        ...
        response['Content-Disposition'] = "attachment; filename=Incorporados_por_organismos_sma_(%s)_(%s).xlsx" % (provincia_usuario,
                                                                                                     anno_actual)
        ...
        response['Content-Disposition'] = "attachment; filename=Incorporados_por_organismos_sma_(%s).xlsx" % anno_actual

        PROVINCIAS = Provincia.objects.all()
        # MUNICIPIOS = Municipio.objects.all()
        LICENCIADOS_SMA = LicenciadosSMA.objects.filter(fecha_registro__year=anno_actual).values("incorporado", "organismo_id", "ubicacion_id",
                                                                                                 "municipio_residencia__provincia_id") | \
                          LicenciadosSMA.objects.filter(activo=True).values("incorporado", "organismo_id", "ubicacion_id",
                                                                            "municipio_residencia__provincia_id")
