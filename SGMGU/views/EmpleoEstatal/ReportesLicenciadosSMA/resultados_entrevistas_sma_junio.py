...
def resultados_entrevistas_sma_junio(request):
    ...
    if categoria_usuario == 'dmt':
        response[
            'Content-Disposition'] = "attachment; filename=Resultados_entrevistas_sma_(Junio)(%s)_(%s).xlsx" % (municipio_usuario,
                                                                                                anno_actual)
        ...
        response['Content-Disposition'] = "attachment; filename=Resultados_entrevistas_sma_(Junio)(%s)_(%s).xlsx" % (provincia_usuario,
                                                                                                     anno_actual)
        ...
        response['Content-Disposition'] = "attachment; filename=Resultados_entrevistas_sma_(Junio)(%s).xlsx" % anno_actual

        PROVINCIAS = Provincia.objects.all()
        MUNICIPIOS = Municipio.objects.all()
        LICENCIADOS_SMA = LicenciadosSMA.objects.filter(activo=True, mes_entrevista='Junio').values("recibio_oferta", "acepto_oferta", "ubicacion", "causa_no_aceptacion")
