...
def ubicacion_general_sma_diciembre(request):
    ...
    if categoria_usuario == 'dmt':
        response[
            'Content-Disposition'] = "attachment; filename=Ubicacion_general_sma_Diciembre_(%s)_(%s).xlsx" % (municipio_usuario,
                                                                                                anno_actual)
        ...
        response['Content-Disposition'] = "attachment; filename=Ubicacion_general_sma_Diciembre_(%s)_(%s).xlsx" % (provincia_usuario,
                                                                                                     anno_actual)
        ...
        response['Content-Disposition'] = "attachment; filename=Ubicacion_general_sma_Diciembre_(%s).xlsx" % anno_actual

        PROVINCIAS = Provincia.objects.all()
        MUNICIPIOS = Municipio.objects.all()
        LICENCIADOS_SMA = LicenciadosSMA.objects.filter(mes_entrevista='Diciembre', fecha_registro__year=anno_actual).values("incorporado", "sexo", "ubicacion") | \
                          LicenciadosSMA.objects.filter(mes_entrevista='Diciembre', activo=True).values("incorporado", "sexo", "ubicacion")
