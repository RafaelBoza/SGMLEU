...
def ubicacion_general_sma_junio(request):
    ...
    if categoria_usuario == 'dmt':
        response[
            'Content-Disposition'] = "attachment; filename=Ubicacion_general_sma_Junio_(%s)_(%s).xlsx" % \
        ...
        response['Content-Disposition'] = "attachment; filename=Ubicacion_general_sma_Junio_(%s)_(%s).xlsx" % \
        ...
        LICENCIADOS_SMA = LicenciadosSMA.objects.filter(municipio_residencia__provincia=provincia_usuario,
                                                        mes_entrevista='Junio', fecha_registro__year=anno_actual).values("incorporado", "sexo", "ubicacion") | \
                          LicenciadosSMA.objects.filter(municipio_residencia__provincia=provincia_usuario,
                                                        mes_entrevista='Junio', activo=True).values("incorporado", "sexo", "ubicacion")
    else:
        response['Content-Disposition'] = "attachment; filename=Ubicacion_general_sma_Junio_(%s).xlsx" % anno_actual

        PROVINCIAS = Provincia.objects.all()
        MUNICIPIOS = Municipio.objects.all()
        LICENCIADOS_SMA = LicenciadosSMA.objects.filter(mes_entrevista='Junio', fecha_registro__year=anno_actual).values("incorporado", "sexo", "ubicacion") | \
                          LicenciadosSMA.objects.filter(mes_entrevista='Junio', activo=True).values("incorporado", "sexo", "ubicacion")
