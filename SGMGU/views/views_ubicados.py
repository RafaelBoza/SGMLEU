...
def m_ubicados(request,filtro):
    ...
    # disponibles = DisponibilidadGraduados.objects.filter(fecha_registro__year=datetime.datetime.today().year, centro_estudio__provincia__id=4)
    #
    # cantidad = 0
    # lista_ci = []
    # for d in ubicados.filter(centro_estudio__provincia__id=4):
    #     if disponibles.filter(ci=d.ci) > 0:
    #         cantidad += 1
    #         lista_ci.append(d.ci)
    # print cantidad
    # print lista_ci
...
@login_required
@permission_required(['administrador', 'especialista'])
def modificar_ubicacion_laboral(request, id_ubicacion):
    print id_ubicacion
    ubicacion=UbicacionLaboral.objects.get(id=id_ubicacion)
    if request.method == 'POST':
